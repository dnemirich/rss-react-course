...
export const Card = ({item}: Props) => {
  return (
    ...
    <p className={'text-lg'}>
      ...
      <p className={'text-lg'}>
        <span className={'font-bold text-xl'}>Description:</span> {item.description.length > 0 ? item.description : 'No description'}
      </p>
      {item.additionalData &&
        Object.entries(item.additionalData).map(([key, value]) => (
          <p className={'text-lg'} key={key}>
            <span className={'font-bold text-xl capitalize'}>{key.split('_').join(' ')}:</span> {value}
          </p>
        ))
      }
    </li>
